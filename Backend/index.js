...
app.post("/upload-grades", async(req, res) => { //Grades
  const csv = require("fast-csv");
  const fs = require("fs");
  const path = require("path");
  const filename = Date.now() + "_" + req.files.file.name;
  ...
  try {
    fs.createReadStream(uploadPath)
      .pipe(csv.parse({ headers: ['ordernumber', 'psasub', 'cert','grade','description','card'],ignoreHeaders: true }))
      .on("error", err => console.log(err))
      .on("data", row => {
        allGrades.push({ ...row });
      })
...
if (uniqueEmails) {
  ...
  let message = {
    ...
    text:
      "Hi, Your grades are now ready for Order No:  4849 "
  };
      ...
      const autocompleteData = await User.aggregate([
        ...
        },
        {
          $project: {
            ...
            PSAUpchargeAmmount:
              "$orders.grades.PSAUpchargeAmmount",
...
NumberOfKicksfromservicelevel:'$orders.NumberOfKicksfromservicelevel',
  ...
  try {
    const userdetails = await User.aggregate( [
      ...
      },
      {
        $match: {
          "orders.orderStatus":
            "Awaiting Customer Pickup", //Awaiting Customer Pickup
        },
      },
      {
        $project: {
  ...
  try {
    const userdetails = await User.aggregate( [
      ...
      },
      {
        $match: {
          "orders.orderStatus":
            "Requested Returns", //Requested Returns
        },
      },
      {
        $project: {
...
app.delete('/delete-review', async (req, res) => {
  try {
    const id = req.body.id;
  await ureviews.findByIdAndRemove(id)
    .then((data) => {
      if (!data) {
        res.status(404).send({
          message: `Cannot delete`,
...
app.patch("/updatePsa-subtracker", async (req, resp) => {
  try {
    ...
    const response = await axios.get(`https://api.psacard.com/publicapi/order/GetSubmissionProgress/${psasubnum}`, { headers });
