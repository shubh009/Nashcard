    ...
    const columns = [
          ...
          },
        {
          ...
          },
          {
            ...
            renderCell: (row) =>
          <td>
          <Link to="#" onClick={(e)=>handleUpdate(row.row.orderid)} className="">Mark as Shipped</Link>
          </td>
          },
      ];
...
const rows = paidOrders.map((element, index) => ({
  ...
  const getdataPaid = async () => {
    const res = await fetch("http://localhost:5000/get-paid-orders", {
      method: "POST",
      headers: {
...
const handleUpdate = async (orderid) => {
    const getUpdate = async () => {
      const res = await fetch(`http://localhost:5000/mark-as-shipped`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        }, body: JSON.stringify({
