...
const index=()=>{
     ...
     const getGradeList=async(e)=>{
      ...
      let result = await fetch('http://localhost:5000/order-status',{
          method:"POST",
          body: fbody,
          headers: {
     ...
     const handleFileSubmit = (e)=>{
      ...
      if (file !== null) {			
        ...
        Axios.post("http://localhost:5000/upload-grades", formValue, {
          headers: {
     ...
     const handleMailSend = (e)=>{
      ...
      handleClose1()
      const getMail = async () => {
        const res = await fetch("http://localhost:5000/get-uploaded-grades-for-mail", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const getdataUpdate = async () => {
    const res1 = await fetch("http://localhost:5000/get-uploaded-grades", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
const columns = [
    ...
    },
  {
...

    {
      ...
      renderCell: (row) =>
    <td>
    <td><Link to="/admin/order-details/" className="text-danger" onClick={()=>orderDetails(row.row.userid, row.row.ordernumber, row.row.grcompanyname)}> Order Details</Link></td>
    </td>
    },
    {
      field: "alert",
      ...
      renderCell: (row) =>
    <td>
    <td><Link to="#" className="text-danger" onClick={()=>orderDetails(row.row.userid, row.row.ordernumber, row.row.grcompanyname)}>Alert V2</Link></td>
    </td>
    },
    {
      field: "ordertotal",
  ...
  showColumnVerticalBorder: true,
    ...
    return(
           ...
           <Modal.Body className="px-4 py-4">
               <div className="row">
                 <div className="col-lg-6">
                  ...
                  onChange={(e)=>setorderid(e.target.value)} onKeyDown={(e)=>searchOrderId(e.target.value)}></input>
      ...
      <Card.Body style={{cursor: 'pointer' }} onClick={()=>handleSuggestion(el.orderid)}>{el.orderid}</Card.Body>
        ...
        <Modal.Body>WoW! All records from the CSV file has uploaded. <br></br>
Now Do you want to send mail to each user for grade popped? 
If yes please click the "SEND NOW" button or click the "Close" button to dismiss the popup</Modal.Body>
        <Modal.Footer>
