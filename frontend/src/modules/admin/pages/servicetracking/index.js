...
const index=()=>{
  ...
  const getdata = async () => {
    const res = await fetch(`http://localhost:5000/order-search-service-tracking`, {
      method: "POST",
      headers: {
...
const getdata = async () => {
  const res = await fetch("http://localhost:5000/service-level-tracking-order-autocomplete", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const getdata1 = async () => {
  const res = await fetch("http://localhost:5000/get-data-service-tracking", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
                  ...
                  <div className="col-lg-12">
                    <div className="Dashboardsec box-shadow helpform">
                         <div className="row">
                             ...
                             <div>
                             <DataGrid
                            style={{ fontWeight: "400", marginTop:"25px", marginBottom:"15px" }}
                            components={{
                              Toolbar: GridToolbar,
                            }}
                            density="compact"
                    ...
                    <div className="Dashboardsec box-shadow helpform">
                         ...
                         <div className="col-lg-8">
                                <div className="form-group">
                                    ...
                                    <input type="textbox" onChange={(e)=>setServiceLevel(e.target.value)} name='servicelevel' value={getServiceLevel} onKeyDown={(e)=>searchOrderId(e.target.value)} className="form-control" placeholder="Enter 2 or more characters"></input>
                  ...
                  <div className="col-lg-12">
                    <div className="Dashboardsec box-shadow helpform">
                         <div className="row">
                             <div className="col-lg-8">
                              <div className="Topl">
                                 <span className="heading-level-2">Orders Late to PSA</span>
                              </div>
                             </div>
                        ...
                        <Table striped bordered hover className="mt-4">
                           <thead>
                               <tr>
                                ...
                                {searchDatas ? searchDatas.map((el) => {
                          return <tr key={el._id}>
                             ...
                             <td>{el.cardQty}</td>
                             <td>{el.orderTotal}</td>
                             <td><Link to='/admin/order-details/' className="text-danger" onClick={()=>orderDetailsSearch(el.userid, el.orderid, el.grcompanyname)}>Order Details</Link></td>
                           </tr>
                          }) : null}
                            </thead>
                        </Table>
