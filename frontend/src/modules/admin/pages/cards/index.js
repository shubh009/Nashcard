...
const Cards=()=>{
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (row) =>
      <td>
      <Link to='/admin/card/card-details/' className="btn btn-danger" onClick={()=>cardDetails(row.row._id)}>Card Details</Link>
      </td>
    },
    {
      field: "orderdetails",
      ...
      renderCell: (row) =>
      <td>
      <Link to='/admin/order-details/' className="btn btn-primary" onClick={()=>orderDetails(row.row.userid, row.row.orderid, row.row.grcompanyname)}>Order Details</Link>
      </td>
    },
  ];
  let d
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
  const getdata = async () => {
    const res = await fetch(`http://localhost:5000/card-search`, {
      method: "POST",
      headers: {
...
useEffect(()=>{
  const getdata = async () => {
    const res = await fetch("http://localhost:5000/psasub-autocomplete", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const getdata1 = async () => {
    const res = await fetch("http://localhost:5000/order-autocomplete", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
                    ...
                    <div className="Dashboardsec box-shadow helpform">
                         ...
                         <div className="col-lg-4">
                                <div className="form-group">
                                    ...
                                    <input type="textbox" value={orderid} name='orderid' onChange={(e)=>setOrderid(e.target.value)} onKeyDown={(e)=>searchOrderId1(e.target.value)} className="form-control"></input>
                                    {orderIdMatch1 ? orderIdMatch1.map((el,index)=>{
                   return <Card title={el.orderid} key={index}>
      <Card.Body style={{cursor: 'pointer' }} onClick={()=>handleSuggestion1(el.orderid)}>{el.orderid}</Card.Body>
    </Card>
                  }):null}
                                </div>
                                </div>
                                <div className="col-lg-4">
                                    <div className="form-group">
                                      <label className="fw-bold mb-2">PSA Sub#</label>
                                           ...
                                           <input name="psasub"  value={psasub} onChange={(e)=>setPsasub(e.target.value)} onKeyDown={(e)=>searchOrderId(e.target.value)} type="textbox" className="form-control"></input>
                                           {orderIdMatch ? orderIdMatch.map((el,index)=>{
                   return <Card title={el.orderid} key={index}>
      <Card.Body style={{cursor: 'pointer' }} onClick={()=>handleSuggestion(el.orderid)}>{el.orderid}</Card.Body>
    </Card>
                  }):null}
                                ...
                                <div className="col-lg-4 mt-4">
                                   <Link onClick={handleSubmit} className="submitbtn mt-1 float-start text-center">Search</Link>
