    ...
    handleClose()
      const creationdate = date
          const getdata1 = async () => {
            ...
            const res = await fetch(`http://localhost:5000/create-subnum`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              }, body: JSON.stringify({
                creationdate,subnumber
              }),
            });
...
const getdata = async () => {
  const res = await fetch("http://localhost:5000/get-status-subtracker", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const getdataOrdersRecieved = async () => {
  const res = await fetch("http://localhost:5000/get-orders-recieved", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const getdataPsaOrders = async () => {
  const res = await fetch("http://localhost:5000/get-psa-orders-for-subtracker", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const statusUpdate = async (psasubnum)=>{
  ...
  const getdataWithSubnum = async () => {
            ...
            const creationdate = date
            const res = await fetch(`http://localhost:5000/updatePsa-subtracker`, {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
              }, body: JSON.stringify({
...
const columns = [
    ...
    },
  {
    ...
    },
    {
      ...
      renderCell: (row) =>
    <td>
    <Link to="#" onClick={()=>statusUpdate(row.row.subnumber)} className="">Request Updated Status</Link>
    </td>
    },
];
...
const columnsPsaOrders = [
    ...
    },
  {
    ...
    },
    {
      ...
      renderCell: (row) =>
    <td>
    <td><Link to="/admin/order-details/" className="text-danger" onClick={()=>orderDetails(row.row.userid, row.row.orderid, row.row.grcompanyname)}> Order Details</Link></td>
    </td>
    },
];
...
const columnsOrderRecieved = [
    ...
    },
  {
    ...
    },
    {
      ...
      renderCell: (row) =>
    <td>
    <td><Link to="#" className="text-danger" onClick={()=>orderDetails(row.row.userid, row.row.orderid, row.row.grcompanyname)}> Note As Problem</Link></td>
    </td>
    },
...
<div className="container-fluid " id="admin">
              ...
              <div className="col-lg-10 noleftrightpadding">
                 ...
                 <div className="Dashboardsec box-shadow helpform">
                    <div className="row">
                      ...
                      <p>Click the <b>Update Non-Complete All Subs</b> to pull updates on every sub.</p>
                      <p>Or, use the table to below to search for an existing sub and pull information manually via the <b>Request Updated Status</b> button</p>
                      <p>Use the filters to filter a specific category of status OR to remove complete subs</p>
                      <p>
                      If you DO NOT FIND the sub your are looking for click the <b> + Add New Submission</b>
                      </p>
                      <p className="text-justify">
                        <b>Not all subs are available and the table below works every time we receive a new confirmation email that the submission number has been created. Subs forms prior to Feb. 2022 will NOT show in the table unless manually entered.  </b>
                      </p>
                 ...
                 <div className="Dashboardsec box-shadow helpform">
                   <div className="row ">
                    <div className="col-lg-4">
                        <div className="Topl">
                            <span className="heading-level-2 mb-3">PSA Sub Tracker</span>
                        </div>
                    </div> 
                    <div className="col-lg-3">
                    <button className="submitbtn text-center" onClick={handleShow} > Add New Submission</button>
                    </div>
        ...
        <Modal.Body>
            <div className="form-group">
               ...
               <input   value={date} onChange={handleDate} type="date" className="form-control"></input>
            </div>
            <div className="form-group">
               <label>Sub Number</label>
               <input placeholder="Sub Number"  value={subNum} onChange={handleSubNum} type="number" className="form-control"></input>
            </div>
        </Modal.Body>
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleSubmit} >
            Save Changes
                    ...
                    <div className="col-lg-4">
                       <select value={selectDatas} onChange={handleSelect} className="form-control">
                ...
                <div className="Dashboardsec box-shadow helpform">
                   <div className="row ">
                    <div className="col-lg-5s">
                        <div className="Topl">
                            <span className="heading-level-2 mb-3">Orders Received by Nashcards</span>
                        </div>
                    </div> 
