...
const handleDesc = (e) => {
  ...
  const handleSubmit = async (e) => {
    ...
    handleClose()
    if (isvalid) {
      if (projectTitle !== "" && status !== "" && assignedMember !== "" && description !== "" && date !== "") {
        const dueDate = date
          const getdata1 = async () => {
            const res = await fetch(`http://localhost:5000/create-project`, {
              ...
              headers: {
               ...
               projectTitle,status,description,dueDate,assignedMember
              }),
            });
...
const getdata = async () => {
  const res = await fetch("http://localhost:5000/get-details-project", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const getdataAssignedMember = async () => {
  const res = await fetch("http://localhost:5000/find-member", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const columns = [
    ...
    },
  {
    ...
    },
    {
      ...
      renderCell: (row) =>
    <td>
    <Link to="/admin/edit-project/" onClick={()=>orderDetails(row.row._id)} className="">Edit</Link>
    </td>
    },
  {
    field: "delete",
  ...
  window.confirm(
    ...
    : e.preventDefault()} className="">Delete</Link>
    </td>
  },
];
...
const projectDelete = async (id)=>{
  ...
  const getdataDelete = async () => {
    const res = await fetch(`http://localhost:5000/delete-project`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      }, body: JSON.stringify({
    ...
    return(
                 ...
                 <div className="Dashboardsec box-shadow helpform">
                   <div className="row ">
                    <div className="col-lg-8">
                        <div className="Topl">
                            <span className="heading-level-2 mb-3">Projects Data Table</span>
                        </div>
                    </div> 
                    <div className="col-lg-4">
                        <button className="submitbtn text-center" onClick={handleShow}> Add New Projects</button>
                    </div>    
                   </div>
                   <div className="row">
        ...
        <Modal.Body>
            <div className="form-group">
               ...
               <input placeholder="Project Title"  value={projectTitle} onChange={handleProjectTitle} type="text" className="form-control"></input>
            </div>
            <div className="form-group">
               <label>Set Status</label>
               <select   value={status} onChange={handleStatus} className="form-control">
            ...
            <div className="form-group">
               ...
               <select   value={assignedMember} onChange={handleAssignedMember} className="form-control">
            ...
            <div className="form-group">
               ...
               <input placeholder="Description"  value={description} onChange={handleDesc} type="text" className="form-control"></input>
            </div>
        </Modal.Body>
        <Modal.Footer>
