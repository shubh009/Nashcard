...
const columns = [
     ...
     },
   {
       ...
       renderCell: (row) =>
       <td>
       <Link to="/admin/shipping/psa-submission-details/" onClick={()=>orderDetails(row.row._id,row.row.orderid,row.row.psasub)} className="">Print Packing List</Link>
       </td>
     },
   {
       field: "specificorder",
       ...
       renderCell: (row) =>
       <td>
       <Link to="#" className="">View Specific Order</Link>
       </td>
     },
 ];
 ...
 const handleSearch = async (e)=>{
   ...
   const getData = async () => {
     const res = await fetch(`http://localhost:5000/get-shipping`, {
       method: "POST",
       headers: {
    ...
    return(
                    ...
                    <div className="Dashboardsec box-shadow helpform">
                         <div className="row mb-3">
                             <div className="col-lg-8">
                                <div className="Topl">
                                   <span className="heading-level-2">Enter PSA/SGC Sub for Order Breakdown </span>
                                </div>
                             </div>
                         </div>
                         <div className="row mb-3">
                            <div className="col-lg-5">
                              <input onChange={handleInput} value={orderid} onKeyDown={(e)=>searchOrderId(e.target.value)} type="text" className="form-control"></input>
      ...
      <Card.Body style={{cursor: 'pointer' }} onClick={()=>handleSuggestion(el.orderid)}>{el.orderid}</Card.Body>
                            ...
                            <div className="col-lg-3">
                                <button onClick={handleSearch} className="submitbtn">Search</button>
                            </div>
