...
useEffect(()=>{
  ...
  const getdata = async () => {
    const res = await fetch("http://localhost:5000/get-orders-shipping", {
      method: "POST",
      headers: {
      ...
      <div>
        ...
        {open && (
          <div className="row" style={{height:'100px'}}>
            ...
            {`Cert:${row.cert}`}</Typography>
            <Typography className="col border" style={{fontSize:'12.5px'}}>{`Description: ${row.desc}`}</Typography>
            <Typography className="col border" style={{fontSize:'12.5px'}}>{`Status: ${row.status}`}</Typography>
            <Typography className="col border" style={{fontSize:'12.5px'}}>{`Name: ${row.name}`}</Typography>
            <Typography className="col border" style={{fontSize:'12.5px'}}>{`Grade: ${row.grade}`}</Typography>
            <Typography className="col border" style={{fontSize:'12.5px'}}>{`PsaSub#: ${row.psasub}`}</Typography>
            <Typography className="col border" style={{fontSize:'12.5px'}}>{`Ship/pickup: ${row.shippickup}`}</Typography>
                    ...
                    <div className="Dashboardsec box-shadow helpform">
                         <div className="row mb-3">
                             <div className="col-lg-8">
                                <div className="Topl">
                                   <span className="heading-level-2">PSA Submission Details </span>
                                </div>
                             </div>
                         </div>
                         ...
                         <div className="col-lg-12">
                            ...
                            <div className="box">
                                <div>
                                    <label className="fw-bold">Service Level</label><br></br>
                                    <label>Beckett - Economy</label>
                                </div>
                            </div>
                            <div className="box">
                                <div>
                                    <label className="fw-bold">Date Added to PSA</label><br></br>
                                    <label>06/12/2023</label>
                                </div>
                            </div>
                            <div className="box">
                                <div>
                                    <label className="fw-bold">Local Drop Off and/or Pick Up Location</label><br></br>
                                    <label>Yes, i will pickup and drop off my cards</label>
                                </div>
                            </div>
                            </div>
                         </div>
              ...
              columns={[
                {
                  ...
                  width: 900,
                  renderCell: (params) => (
                    <CustomCellRenderer value={params.value} row={params.row} />
                  ),
                },
                // ...columns,
              ]}
