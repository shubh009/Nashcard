...
const handleSubmitReview = async (e) => {
  ...
  reviewhandleClose()
  if (isvalid) {
    if (companyname !== "" && status !== "" && cardtype !== "" && comment !== "" && submittedcard !== "") {
...
let reviewid = newreviewid
        ...
        const getdataRev = async () => {
          const res = await fetch(`http://localhost:5000/create-review`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            }, body: JSON.stringify({
             companyname,status,cardtype,comment,submittedcard,userid,reviewid,netdv
            }),
          });
...
const getdataServiceLevel = async () => {
  const res = await fetch("http://localhost:5000/get-service-level", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const getdataOrderStatus = async () => {
  const res = await fetch("http://localhost:5000/get-status-order", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
handleClose()
  ...
  if ( cardcount !== "" && serviceLevel !== "" && localpickup !== "" && contact !== "" && DropoffLocation !== "" && orderStatus !== "") {
...
let pricepercard = pricePerCard
      const getdataOrder = async () => {
        const res = await fetch(`http://localhost:5000/create-order`, {
          ...
          headers: {
           ...
           cardcount,pricepercard,DropoffLocation,contact,localpickup,userid,orderid,servicelevel,orderStatus
          }),
        });
        const data = await res.json();
        if (res.status === 422 || !data) {
  ...
  const getdata = async () => {
    ...
    const res = await fetch("http://localhost:5000/user-details-data-with-id", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const getdata1 = async () => {
    const res = await fetch("http://localhost:5000/customers-unpaid-order", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const getdataOrders = async () => {
    const res = await fetch("http://localhost:5000/customers-orders", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const getdataReviews = async () => {
    const res = await fetch("http://localhost:5000/review-orders", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const columns = [
      ...
      },
    {
      ...
      },
      {
        ...
        renderCell: (row) =>
      <td>
     <Link to="/admin/order-details/" className="btn btn-primary" onClick={()=>orderDetails(row.row.userid, row.row.orderid, row.row.grcompanyname,row.row._id)}>Order Details</Link>
      </td>
      },
    {
  ...
  const columnsReview = [
    ...
    },
    {
      ...
      },
      {
        ...
        renderCell: (row) =>
      <td>
     <Link to="#" className="btn btn-primary" onClick={()=>orderDetailsReview(row.row.reviewid)}>Review Order Details</Link>
      </td>
      },
  ];
                        ...
                        <div className="Dashboardsec box-shadow helpform">
                          ...
                          />
                                ...
                                <div className="col-lg-3">
                                   <Link className="submitbtn mt-1 float-start text-center" onClick={handleShow}>Add New Order</Link>
                                 
                                </div>
                            </div>
                            <div className="row">
                                {/* code for order data list */}
                            </div>
                         </div>
                         ...
                         <div className="Dashboardsec box-shadow helpform">
                          ...
                          />
                                ...
                                <div className="col-lg-3">
                                   <Link className="submitbtn mt-1 float-start text-center" onClick={reviewhandleShow}>Add New Review Order</Link>
                                 
                                </div>
                            </div>

                            <div className="row">
                                {/* code for review data list */}
                            </div>
        ...
        <Modal.Body>
          <div className="row">
              <div className="form-group">
                  ...
                  <select onChange={handleServiceLevel} className="form-control">
                     <option value="0">Select Service Level</option>
                     {serviceLevelData? serviceLevelData.map(el=>{
return <option value={el.servicelevel+"-"+ el.pricepercard}  key={el._id}>{el.servicelevel + "         (Price Per Card "+ el.pricepercard+")" }</option>  
                     }):null}
                  </select>
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                  ...
                  <input onChange={handleCardCount} type="text" className="form-control"></input>
              </div>
              <div className="form-group">
                  <label>Status </label>
                  <select onChange={handleOrderStatus} className="form-control">
                     ...
                     {orderstatus?
                      orderstatus.map((el)=>{
                    return    <option value={el.status} key={el._id}>{el.status}</option>
                      })
                    :null}
                  </select>
                  <p>Intended status. If customer has cards, mark as Cards Received. If customer will be dropping them off later, mark as Awaiting Cards</p>
              </div>
              <div className="form-group">
                  <label>Local Pickup/Drop Off? * </label>
                  <ul className="list-unstyled">
                                        <li>
                                          <input id="r1" type="radio" name="radio" value="1" onChange={handlelocalpickup}></input>
                                         <label htmlFor="r1">Yes, I'll drop off and pickup my cards</label>
                                         </li>
                                        <li>
                                          <input id="r2" type="radio" name="radio" value="2" onChange={handlelocalpickup}></input>
              ...
              <div className="form-group">
                  ...
                  <input onChange={handleContact} type="text" className="form-control"></input>
              </div>
              <div className="form-group">
                  <label>Local Drop Off and/or Pick Up Location </label>
                  <select onChange={handleDropOffLocation} className="form-control">
                     <option value="0">Select</option>
        ...
        <Modal.Body>
          <div className="row">
              ...
              <div className="form-group">
                  ...
                  <ul className="list-unstyled">
                                        <li>
                                          <input id="r1" type="radio" name="radio" value='SGC' onChange={handleCompanyname}></input>
                                         <label htmlFor="r1">SGC</label>
                                         </li>
                                        <li>
                                          <input id="r2" type="radio" name="radio" value="PSA" onChange={handleCompanyname}></input>
              ...
              <div className="form-group">
                  ...
                  <input onChange={handleSubmittedCards} type="text" className="form-control"></input>
              </div>
              <div className="form-group">
                  <label>Estimated Net DV</label>
                  <input onChange={handleDV} type="text" className="form-control"></input>
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                  ...
                  <input onChange={handleComments} type="text" className="form-control"></input>
              </div>
